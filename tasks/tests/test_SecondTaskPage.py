...
#     secondtaskpage = SecondTaskPage(browser)
#     region, list_of_orgs, url, title_of_new_page, new_region, new_list_of_orgs = secondtaskpage.RegionCheck()
#     assert region != new_region, setup_logging.error("the region has not changed")
#     assert list_of_orgs[0].get_attribute("textContent") != new_list_of_orgs[0].get_attribute("textContent"), setup_logging.error("the list of organizations has not changed")
#     assert "kamchatskij-kraj" in url, setup_logging.error("incorrect url")
#     assert "Камчатский край" in title_of_new_page.get_attribute("textContent"), setup_logging.error("incorrect web title")